...
int main(int argc, char **argv)
{
	...
	if(opts.mode == 0){
		...
		ndist = g::proc::DirectInquiry(reference, peer, alignment, opts.scale_diff, opts.radius, 1, true);
		EX_TRACE("Feedback-->[%d : %d] in the raw signal\n", alignment[0].second, alignment[alignment.size()-1].second)
		EX_TIME_END("Direct inquiry...")
	}
	else{
		EX_TIME_BEGIN("Fast inquiry...")
		ndist = g::proc::FastInquiry(reference, peer, alignment, opts.scale_diff, opts.K, opts.L, opts.radius, 1, true);
		EX_TRACE("Feedback-->[%d : %d] in the raw signal\n", alignment[0].second, alignment[alignment.size()-1].second)
		EX_TIME_END("Fast inquiry...")
	}
